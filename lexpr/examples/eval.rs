...
pub fn eval(expr: lexpr::Value, env: Gc<Env>) -> OpResult {
    match expr {
        lexpr::Value::Symbol(sym) => {
            env.lookup(&sym).ok_or_else(|| {
                make_error!("unbound identifier `{}'", sym)
            })
        }
        lexpr::Value::Cons(cell) => {
            let (first, rest) = cell.into_pair();
            let op = eval(first, env.clone())?;
            let arg_exprs = proper_list(rest).map_err(|e| make_error!("syntax error: {}", e))?;
            let args = arg_exprs.into_iter().map(|arg| eval(arg, env.clone())).collect::<Result<Vec<_>, _>>()?;
            apply(op, &args)
        }
...
pub fn apply(op: Gc<Value>, args: &[Gc<Value>]) -> OpResult {
    match *op {
        ...
        _ => Err(make_error!("non-applicable object in operator position: {}", op)),
    }
}
