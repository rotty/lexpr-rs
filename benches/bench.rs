...
use lexpr::{from_str, from_str_default, parse};

#[bench]
fn bench_float_parsing(b: &mut Bencher) {
    b.iter(|| black_box(from_str_default("-1.360438755021694e308")));
}

#[bench]
fn bench_parsing_keyword_default(b: &mut Bencher) {
    b.iter(|| black_box(from_str_default("#:some-keyword")))
}
...
fn bench_parsing_keyword_all_styles(b: &mut Bencher) {
    ...
    b.iter(|| black_box(from_str("#:octo :prefix postfix:", &options)));
}
